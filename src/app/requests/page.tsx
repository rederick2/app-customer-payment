...
export default async function AdminRequestsView() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="font-serif text-3xl font-bold tracking-tight mb-2">Solicitudes de Servicio</h1>
        <p className="text-muted-foreground">Gestiona las solicitudes enviadas por tus clientes.</p>
      </div>
      ...
      <div className="grid gap-6">
        {requests && requests.length > 0 ? (
            ...
            return (
              <Card key={request.id} className="border-border/50 shadow-sm overflow-hidden">
                <CardHeader className="bg-muted/30 border-b border-border/50 pb-4 flex flex-row items-center justify-between">
                   <div className="space-y-1">
                     ...
                     <p className="text-xs text-muted-foreground">
                       Recibida el {new Date(request.created_at).toLocaleDateString('es-ES')} a las {new Date(request.created_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit'})}
                     </p>
                   </div>
                ...
                <CardContent className="pt-6">
                      ...
                      {request.on_site_instructions && (
                        <div>
                          <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
                            <MapPin className="h-3.5 w-3.5" /> Instrucciones Especiales
                          </h4>
                    ...
                    <div className="space-y-4 bg-muted/20 p-4 rounded-lg">
                      {request.schedule_date && (
                        <div className="flex items-start gap-3">
                           ...
                           <div>
                             <p className="text-xs font-semibold uppercase text-muted-foreground">Fecha Propuesta</p>
                      ...
                      {request.time_preference && (
                        <div className="flex items-start gap-3">
                           ...
                           <div>
                             <p className="text-xs font-semibold uppercase text-muted-foreground">Horario Ideal</p>
                      ...
                      {request.images && request.images.length > 0 && (
                        <div className="flex items-start gap-3">
                           ...
                           <div>
                             <p className="text-xs font-semibold uppercase text-muted-foreground">Adjuntos</p>
                             <p className="text-sm font-medium">{request.images.length} imágenes</p>
                           </div>
                        </div>
                      )}
                    </div>
                  ...
                  {request.images && request.images.length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-3">Imágenes</h4>
          ...
          <div className="text-center py-12 bg-card border border-border/50 rounded-lg">
            ...
            <h3 className="text-lg font-medium text-foreground">Aún no hay solicitudes</h3>
            <p className="text-muted-foreground mt-1">Cuando tus clientes envíen solicitudes de servicio, aparecerán aquí.</p>
          </div>
        )}
      </div>
    </div>
